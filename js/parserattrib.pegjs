/*Problema: Recursão excessiva, impedindo que a atribuição seja salva antes de
  processar o identificador.
  Ex: A <- X; A. A é processado antes do valor X ser atribuido a A.
  Parser para Álgebra Relacional - Luís A. C. 16/04/2017
  ...
  Use o PEG.js para compilar (pegjs.org)*/
{
    ...
    //Funcao responsavel por fazer o pós-processamento da query
    function prepare(rel){
        ...
        console.log("prepare");
        console.log(rel);
        console.log(querysSalvas); 
    	if (querysSalvas[rel] !== undefined){
        	return querysSalvas[rel];
        }else if(rel.search("SELECT ") !== 0){
    ...
    //Funcao Responsável por salvar a query no map
    function saveQuery(chave, query){
    try{querysSalvas[chave] = query; }catch(e){console.log(e);} 
    	querysSalvas[chave] = query; 
        console.log("save");
        console.log(querysSalvas);
        console.log(chave, query);
    }
...
UnionRelation
 = _ rel:Relacao _ symb:SimboloUnion _ rest:UnionRelation {return prepare(rel)+symb+rest;}
 / rel:Relacao {return prepare(rel)};
